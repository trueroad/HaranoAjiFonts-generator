...
ということを教えていただきました。
このファイルから `vert` をパースすれば横書き用・縦書き用 AJ1 CID
の対照表が得られるはずで、CMap ではなくてこちらを使う方法もあり得ます。
（ 縦書き CMap の方法なら横書き CMap 用に作ったパーサを流用できそうですが、
この GSUB のファイルはいままで使っていたファイルとはまったく形式が異なるため、
新しくパーサを作るなどする必要があります。
ただし、縦書き CMap の方法だと縦書きしか対応できませんが、
GSUB のファイルを使う方法だと縦書き `vert` と同様の方法で、
ルビ `ruby` をはじめとする他のグリフ対照表も作れそうなので、
チャレンジする価値はありそうです。。。）
...
現状では format 14 を削除しているので異字体セレクタを使うことができません。
そのためグリフとしてはすべての漢字が揃っているのですが、
異字体セレクタを要するグリフを Unicode で呼び出すことはできません。
...
必須テーブルではないため削除しています。
そのため OpenType feature を使うことができません。
変換するならば CID の置き換えが必要で、
それなりに複雑な変換を考慮する必要がありそうに見えます。。。
全漢字グリフが揃ったので `jp78`, `jp83`, `jp90` の
対応を考えた方がいいんだろう、、、と思っていますし、
縦書きグリフの対照表ができたら、
OpenType feature の縦書き `vert`, `vrt2` の対応を考えた方がよいだろう、、、
と思っていますので、そうすると変換が必要となります。
なお、「びゃん」や「たいと」などは AJ1 には無いでしょうから、
対応のしようが無いと思います。
